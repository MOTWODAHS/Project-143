...
public class Censor : MonoBehaviour {
	...
	private string checkForWhite(string input)
    {
        Regex re = new Regex("[;\\\\/:*?\"<>|&']");
        string withoutWhite = re.Replace(input, "0");
        withoutWhite = Regex.Replace(withoutWhite," ", "0");
        string withoutWhiteChecked = checkForInterspersedNumbers(withoutWhite);
        if (withoutWhiteChecked.Contains("*"))
        {
            return Regex.Replace(withoutWhiteChecked, "0", " ");
        }
        else
        {
            return input;
